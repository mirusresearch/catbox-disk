...
exports = module.exports = internals.Connection = function DiskCache (options) {
    ...
    const defaults = { cleanEvery:3600000, ignorePatterns:[] };
...
internals.Connection.prototype.get = function (key, callback) {
    ...
    return this.readCacheFile(filepath,callback);
};
...
internals.Connection.prototype.cacheCleanerInit = function (){
    ...
    const runCleaner = function (){
        ...
        walker.on('file', (root, fileStat, next) => {
            ...
            for (let i=0; i<self.settings.ignorePatterns.length; i++){
                let p = self.settings.ignorePatterns[i];
                if (fileStat.name.match(p)){
                    // console.log('skipping:',fileStat.name);
                    return next();
                }
            }
            // console.log('pattern not matched:',fileStat.name);
