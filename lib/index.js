...
internals.testDiskAccess = function (cachePath,callback) {
    ...
    const filepath = Path.join(cachePath,'testDiskAccess.' + rando + '.json');
...
exports = module.exports = internals.Connection = function DiskCache (options) {
    ...
    Hoek.assert(settings.cleanEvery, 'Missing cleanEvery value');
...
internals.Connection.prototype.cacheCleanerInit = function (){
    ...
    const self = this;
    const firstrun = Math.floor(Math.random() * (10000 - 200) + 200);
